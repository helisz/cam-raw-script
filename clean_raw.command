...
# 功能3: 删除RAW文件夹中的RAW文件，只保留与JPG同名的文件（永久删除）
delete_raw_files_permanently() {
    ...
    echo ""
    echo "将永久删除 ${#to_delete[@]} 个RAW文件:"
    for file in "${to_delete[@]}"; do
        echo "  $(basename "$file")"
    done
    
    echo ""
    read -p "确认永久删除? (y/n): " confirm
...
# 功能4: 删除RAW文件夹中的RAW文件，只保留与JPG同名的文件（移到回收站）
clean_raw_files() {
    local raw_dir="$SCRIPT_DIR/RAW"
    ...
    echo ""
    echo "将移动 ${#to_delete[@]} 个RAW文件到回收站:"
    for file in "${to_delete[@]}"; do
        echo "  $(basename "$file")"
    done
    
    echo ""
    read -p "确认移动到回收站? (y/n): " confirm
    if [ "$confirm" != "y" ] && [ "$confirm" != "Y" ]; then
        echo "已取消。"
        return 0
    fi
